...
```
lib.mkSystem                              — darwinSystem ビルダー (base + claude-code + shared-scripts 全部入り)
...
mkSystem は base, claude-code, shared-scripts を自動で含む。`modules` で追加/上書き:
...
│   └── HM (user identity + SOPS条件付き + base + claude-code + shared-scripts)
