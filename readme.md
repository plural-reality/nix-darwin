...
3. `./apply` を実行 (shim: `nix run .#apply` → downstream が lock している upstream `packages.apply` を実行)
...
| AI プロンプト・スキル | **このリポジトリ** (`prompt/`, `modules/claude-code.nix`, `docs/agent-environment-alignment.md`) |
...
├── AGENTS.md                     — Codex 向け repo rule adapter
├── CLAUDE.md                     — Claude Code 向け repo rule adapter
├── docs/
│   └── agent-environment-alignment.md — Claude/Codex projection boundary
