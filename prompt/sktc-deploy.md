...
│   │                                    #   (GitHub deploy key, webhook HMAC secret, cachix auth token)
...
| アプリデプロイ (通常: `nix build` → `cachix push` → `git push`) | **Cachix auth token + SSH key (git push)** | 頻繁 |
| アプリデプロイ (フォールバック: `colmena apply`) | **SSH private key のみ** | 緊急時 |
...
# (開発者が事前に cachix push 済みであることが前提)
