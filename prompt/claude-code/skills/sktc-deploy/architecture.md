...
Developer Mac (aarch64-darwin / x86_64-linux)
  ...
  ├─ nix build + cachix push → Cachix Binary Cache (任意の pre-warm)
  ├─ git push            → GitHub Webhook → EC2 self-deploy (通常運用)
  └─ colmena apply       → SSH 経由で直接デプロイ (手動 / フォールバック)
                               │
                               ▼
                      EC2 (NixOS, Community AMI)
                        ├─ Cachix substituter ... cache hit 時に closure を pull
                        ├─ Local build       ... cache miss 時のみ
                        ├─ Database          ... EBS に永続化
                        ├─ Application       ... ビルド済みバイナリ or スクリプト
                        ├─ nginx             ... TLS + webhook プロキシ
                        ├─ webhook binary    ... GitHub push → self-deploy
                        ├─ colmena           ... apply / apply-local
...
- **Cachix Binary Cache による転送**: ローカル deploy は build 成果物を Cachix に push し、Self-Deploy は cache hit 時に pull、cache miss 時は EC2 上でビルド後に push-back できる。SSH 経由の `nix-copy-closure` 転送を減らせる。
- **Self-Deploy Webhook**: EC2 が GitHub webhook を直接受信し `colmena apply-local` で自己デプロイする。事前の `cachix push` は高速化であって前提条件ではない。
...
- **デプロイと最適化の分離**: deploy の正しさは cache の温まり具合や source filtering に依存しない。Cachix と `nix-filter` は速度最適化であり、成功条件ではない。

### デプロイパイプライン（統一モデル）

ローカル deploy と Self-Deploy は実行主体だけが異なり、踏むステップは同一である。

| Step | ローカル deploy | Self-Deploy |
|------|----------------|-------------|
| 1. Resolve dependencies | SSH 鍵, Cachix token をローカルで自動解決 | runtime secrets を EC2 上で自動解決 |
| 2. Build closure | 開発者マシン | EC2（cache miss 時のみ） |
| 3. Publish cache | Cachix push | Cachix push-back |
| 4. Activate | `colmena apply` | `colmena apply-local` |
| 5. Verify | smoke test + revision 検証 | smoke test + revision 検証 |
| 6. Rollback | 失敗時に remote rollback | 失敗時に自動 rollback |

`Cachix` の pre-warm、SSH 多重化、`nix-filter` はこのパイプラインを高速化するための付加物であり、デプロイそのものの前提条件ではない。
...
| ビルド転送 | Cachix Binary Cache | ローカル deploy は push、Self-Deploy は cache hit 時に pull、cache miss 時は build 後に push-back。 |
| ビルド | ローカル (Mac Linux Builder) + EC2 fallback | ローカル deploy は開発者マシンでビルド。Self-Deploy は cache miss 時だけ EC2 でビルドする。 |
...
│   └── version.nix                      # NixOS revision endpoint (/.well-known/version)
├── tests/smoke/
...
| `nixos/version.nix` | plaintext, git | NixOS revision endpoint (configurationRevision を HTTP 配信) |
...
| Self-Deploy: staging (自動) | なし（webhook + runtime secrets, cache hit 時 pull / miss 時 local build + push-back） | 毎 main push |
| Self-Deploy: production (自動) | なし（webhook + runtime secrets, cache hit 時 pull / miss 時 local build + push-back） | 毎タグ push |
