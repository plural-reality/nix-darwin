...
## 既存資料のレビュー化と配布（同じHTMLで両立）

**同じデッキ/資料HTMLが「レビュー面」と「配布物」の両方を賄う**。美は CSS に宿るので Slides ネイティブに変換せず、HTMLの描画をそのまま使う。2本のプロデューサ script:

- **レビュー化** = `scripts/deck-to-review.py deck.html --root .slide-wrapper` → 元HTMLに要素アノテーション engine＋回収オーバーレイ(du- 名前空間・fixed で元レイアウト非破壊)を注入した `deck.review.html` を出す。**semantic な実要素**を ⌘/Ctrl+クリックで注釈できる。ローカル配信(askpage_server)でも、`/api/publish` に raw-html で上げて **review.plural-reality.com のトークンURL** で共有でも動く(同一 collect 契約)。オーバーレイは canonical テーマトークンを inline 消費(SPA と同一 token source)。
- **配布** = `plural-reality-design-system/scripts/deck-to-pdf.py deck.html` → 各スライド=1ページの PDF(Chrome headless print・外部フォントlink除去・`print-color-adjust:exact` でダーク背景保持)。フォント/レイアウトをピクセル忠実に運ぶ。**Google Slides はHTMLインポート不可・ネイティブ再構築はCSSの美に勝てない**([[reference_gws_google_doc_creation]] Slides節)ので、Slidesの箱が要る時のみ PDF各頁→PNG→全面画像で貼る(非編集)。
- **統一ハーネスとの関係(衝突回避)**: 契約正本 = `~/Developer/reviewable-html-workbench/UNIFIED-HARNESS.md`。§7 でデザイン単一テーマ(§5)は thread 0adc431e 所有・review-page SPA/backend は d6bfc994 所有。disposable-ui 側は **canonical トークンを inline・collect schema を emit** して契約に載るだけ(本体HTMLは自由=Q2決定)。**SPA/テーマ本丸は触らない**。
