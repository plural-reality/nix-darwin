...
# 起草前の文体コンテキスト（CRM人物別ルール＋本人の実送信メール）
$SCRIPT style "recipient@example.com"
```
...
起草前に、宛先メールアドレスを使って必ず `style` を実行する。文体の優先順は、**Scrapboxの人物別humanルール → 本人が同じ宛先へ実際に送った直近メール → autoルール**。送信済みメールだけを例にし、下書きやAI生成案を本人文体として混ぜない。過去例の内容・事実をコピーするのではなく、挨拶、距離感、文長、段落、締め方だけを参照する。
