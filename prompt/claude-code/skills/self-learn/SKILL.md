...
browser/computer useで同じ手順を繰り返した、失敗した、又はCLI/MCPへの置換を依頼された場合に行う。現在のタスクの証拠を先に使い、過去の対話全体を毎回走査しない。月次監査など履歴が依頼されたときは`prompt-review`のtool使用監査で期間を絞る。

1. 操作の目的、対象の種類、画面を選んだ理由、成功/失敗を確認した元の証拠を短く記録する。ツール名の出現は実行成功や無駄の証拠ではない。
2. `browser-automation/references/semantic-routing.md`に従い既存の専用skill・CLI/API・typed MCPを確認する。対象アカウント、read/mutation、stable ID、readback、再試行契約まで適合するか調べる。汎用shell/JSを包むだけのMCPは追加しない。
3. 候補を`replace`（同じ目的を既存semantic操作で実現）、`hybrid`（データ操作を移して視覚確認を残す）、`keep_ui`、`unverified`に分ける。ブラウザを減らす件数だけを最適化しない。
4. 承認済みの可逆的な改善はNix正本の最寄りskill/scriptに小さく反映する。新しい権限・認証・対外操作が必要なら、その境界直前まで準備する。規約をmemoryへ重複コピーしない。未検証候補は現在の作業領域に残し、確定した知識にしない。
5. 読取りは現在の正本との一致、更新は隔離fixtureで2回目no-op、timeout後の照合、権限エラー時停止を規模に応じて確認する。指示の大幅変更は`empirical-prompt-tuning`で別実行者に評価させる。hold-outを含め、結果正確性・UI回数・retry・時間・未確認を別々に記録する。fixtureの成功を本番の冪等保証へ昇格させない。
6. source保存、検証、Nix出力、live投影を分けてreadbackする。将来セッションで効くという主張にはlive投影の証拠が要る。実行できなかった境界と次の操作を具体的に残す。

既存Stop hookがある環境ではこの手続きを利用する。hookのないCodex環境は共有操作ルールによるタスク終端の呼び出しと明示レビューが入口であり、常駐監視や定期自動実行が追加されたとは扱わない。
