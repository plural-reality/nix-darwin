...
# meguro-pool-update skill が公式PDFを取得→解釈→schedule.json→evkit calendar(EventKit洗い替え)。
...
EVKIT="/etc/profiles/per-user/tkgshn/bin/evkit"   # 唯一の書込み窓口。apply.swift を直接叩くと TCC 責任プロセスが claude になり許可が失効する
...
if [[ "${1:-}" == "--check" ]]; then
  ...
  echo "evkit:     $([[ -x "$EVKIT" ]] && echo OK || echo MISSING) ($EVKIT)"
  echo "evkitd:    $("$EVKIT" status 2>&1 | head -c 120)"
...
PROMPT='meguro-pool-update skill を実行して。目黒区民センター体育館プールの当月(取得できれば翌月も)個人利用スケジュールを公式PDF(indoorpool_nittei.html から center-jpn PDF を解決)から取得し、「往復コース(ラップ用3・4)が使える時間だけ」のルールで汎用イベントJSON(mode=replace-month, defaultLocationつき)を作り、`evkit calendar < <json>` (apple-calendar の窓口)で iCloudカレンダー「目黒区民プール」を当月洗い替えしてください。夏季(7-9月)は屋外50mも追加。当月PDFが未掲載なら何もせず終了。'

"$CLAUDE" -p "$PROMPT" \
  --allowedTools "Bash(curl:*)" "Bash(pdftotext:*)" "Bash(evkit:*)" "Bash(jq:*)" "Bash(date:*)" "Bash(grep:*)" "Bash(sed:*)" "Bash(mkdir:*)" "Read" "Write" "WebFetch" \
  >> "$LOG" 2>&1
