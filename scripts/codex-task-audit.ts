...
const groupBy = <T>(
  ...
  [...values.reduce<ReadonlyMap<string, ReadonlyArray<T>>>(
    (groups, value) =>
      new Map(groups).set(keyOf(value), [...(groups.get(keyOf(value)) ?? []), value]),
    new Map(),
  ).values()];
...
const clusterByWindow = (
  threads: ReadonlyArray<Thread>,
  windowSeconds: number,
): ReadonlyArray<ReadonlyArray<Thread>> =>
  threads
    .toSorted((left, right) => right.createdAt - left.createdAt)
    .reduce<ReadonlyArray<ReadonlyArray<Thread>>>(
      (clusters, thread) =>
        ((current) =>
          !current || current[0].createdAt - thread.createdAt > windowSeconds
            ? [...clusters, [thread]]
            : [...clusters.slice(0, -1), [...current, thread]])(clusters.at(-1)),
      [],
    );
...
const duplicateFindings = (
  ...
  groupBy(
    ...
    clusterByWindow(objectiveGroup, duplicateWindowHours * 3_600)
      ...
      .flatMap((cluster) =>
        ((cwdGroups) => [
          ...
          ...(cwdGroups.length > 1 ? [duplicateFinding("duplicate_objective", cluster)] : []),
        ])(groupBy(cluster, ({ cwd }) => cwd)),
      ),
  );
  ...
  ((active) =>
    ...
    threads.filter(
      (thread) =>
        ...
        !isScheduled(thread) &&
        !isSynthetic(thread) &&
        options.now - thread.recencyAt <= options.lookbackDays * 86_400,
    ),
  );
...
const valueOptions: ReadonlyArray<string> = [
  "--format",
  "--limit",
  "--stale-hours",
...
const unknownArgument = (argv: ReadonlyArray<string>): string | undefined =>
  argv.find(
    (argument, index) =>
      !["--current", "--help", "-h"].includes(argument) &&
      !valueOptions.includes(argument) &&
      !(index > 0 && valueOptions.includes(argv[index - 1] ?? "")),
  );
export const parseArgs = (argv: ReadonlyArray<string>): Result<Args> =>
  ((unknown) =>
    unknown
      ? err(`unknown argument: ${unknown}`)
            ...
            !["id", "title", "cwd", "archived", "created_at", "recency_at", "source", "first_user_message", "agent_role"]
              ...
              : ((rows) => (database.close(), ok(rows)))(
                  database.prepare(`SELECT
                    ...
                    CASE WHEN first_user_message LIKE 'Automation:%' THEN 'scheduled' ELSE 'general' END AS mode
                  FROM threads
                  WHERE archived = 0
                    AND title <> ''
